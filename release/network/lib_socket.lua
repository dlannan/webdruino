...
if util.isWin then
	function errno()
		return s.WSAGetLastError()
	end
	function errortext(err)
		return util.win_errortext(err)
	end
	function initialize()
		if wsastartup_done == true then return end
		local wsadata
		--if is64bit then
...
function set_reuseaddr_opt(fd)
    setopt[0] = 1
    local rc = s.setsockopt(fd,
        s.SOL_SOCKET,
        s.SO_REUSEADDR,
        ffi.cast("const char *", setopt),
        ffi.sizeof("int32_t"))
    if rc ~= 0 then
       return -1
    end
...
function setsockopt(socket, level, option_name, option_value)
	...
	--local arg_c = ffi.new("char[4]", option_value)
	local option_len = 4 --ffi.sizeof(arg_c)
	return s.setsockopt(socket, level, option_name, ffi.cast("const char *", arg_c), option_len)
...
function inet_pton(family, szAddrString, pAddrBuf)
	return s.inet_pton(family, szAddrString, pAddrBuf)
end
--[[
