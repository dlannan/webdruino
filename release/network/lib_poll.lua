...
local in_arg, out_arg, close_arg, error_arg
...
function modify_fd(fd, events)
	local idx = fd_arr_index(fd)
	if idx < 0 then
		-- is old fd number, is ok when we reuse addresses ???
		print("ERR: FD was not found cannot modify (poll.add_fd): idx="..idx..", fd="..fd)
		print(fd_arr_show())
		return -1
	end
	fds[idx].fd 			= fd -- set C struct pollfd field fd, same as fds[nfds].fd = fd
	fds[idx].events 	= events -- bor(C.POLLIN, C.POLLOUT, C.POLLRDHUP)
	fds[idx].revents 	= 0 -- no need to set

	if debug_level > 0 then print("  poll.modify_fd: fd="..fd..", nfds="..nfds) end
end
...
function in_callback_set(func, arg)
	in_callback = func
	in_arg = arg
end
function out_callback_set(func, arg)
	out_callback = func
	out_arg = arg
end
function close_callback_set(func, arg)
	close_callback = func
	close_arg = arg
end
function error_callback_set(func, arg)
	error_callback = func
	error_arg = arg
end
...
local function poll_event_nodebug(evt, fd)
  ...
  if band(evt, POLLHUP) ~= 0 then
    close_callback(close_arg, fd, evt)
  elseif band(evt, POLLIN) ~= 0 then
    in_callback(in_arg, fd, evt)
  elseif band(evt, POLLOUT) ~= 0 then
    out_callback(out_arg, fd, evt)
  elseif band(evt, POLLNVAL) ~= 0 then
    error_callback(error_arg, fd, "POLLNVAL")
  elseif band(evt, POLLERR) ~= 0 then
    error_callback(error_arg, fd, "POLLERR")
  end
end
...
local function poll_event_debug(evt, fd, i)
  ...
  if band(evt, POLLHUP) ~= 0 then -- usually POLLIN | POLLHUP, but we don't want C.POLLIN
    -- POLLHUP, output only
  ...
  elseif band(evt, POLLNVAL) ~= 0 then
    ...
    error_callback(in_arg, fd, "POLLNVAL")
  elseif band(evt, POLLERR) ~= 0 then
    -- POLLERR, output only
...
function debug_level_set(level)
  ...
  -- print("poll debug level:  "..debug_level)
end
...
function poll()
	...
	if ret == -1 then
		--print(pollCount..". poll, nfds="..nfds, "fd: "..fds[1].fd, "event:"..fds[1].events, nfds, timeout )
